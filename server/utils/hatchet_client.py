...
def get_hatchet_client() -> Hatchet:
    ...
    if _hatchet_client is None:
        ...
        _hatchet_client = Hatchet(
            debug=os.getenv('LEGACY_USE_DEBUG', '0') == '1'
        )
...
class HatchetJobManager:
    ...
    def __init__(self):
        self.hatchet = get_hatchet_client()
        self._workflow = None
    ...
    async def enqueue_job(self, job: Job, tenant_schema: str) -> str:
        ...
        try:
            ...
            with with_db(tenant_schema) as db_session:
                ...
                logger.info(f"Job {job.id} status updated to QUEUED for tenant {tenant_schema}")
            ...
            # Run the workflow
            workflow_run = await self._workflow.aio_run(input_data)
            
            logger.info(f"Job {job.id} enqueued in Hatchet with run ID: {workflow_run.workflow_run_id}")
...
# Global instance
hatchet_job_manager = HatchetJobManager()