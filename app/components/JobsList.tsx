...
import type { Job, Target, APIDefinition, PaginatedJobsResponse } from '../gen/endpoints';
...
const JobsList = () => {
  ...
  const fetchFilterOptions = async () => {
    try {
      ...
      setTargets((targetsResponse || []) as Target[]);

      // Process API definitions response
      const apisList = Array.isArray(apisResponse) ? (apisResponse as APIDefinition[]) : [];
      setApis(apisList);
    } catch (err) {
  ...
  const handleFilterChange = (event: SelectChangeEvent<string>) => {
    const name = event.target.name as 'status' | 'target_id' | 'api_name';
  ...
  const handleRowClick = (event: React.MouseEvent, job: Job) => {
  ...
  const handleCancelJob = async (event: React.MouseEvent, job: Job) => {
    ...
    try {
      ...
      await cancelJob(job.target_id, job.id as string);
            ...
            {loading ? (
              ...
              jobs.map(job => (
                ...
                >
                  ...
                  <TableCell>
                    <Chip label={job.status} color={getStatusColor(job.status as string)} size="small" />
                  </TableCell>
                  <TableCell>
                    {typeof job.duration_seconds === 'number' ? `${Math.round(job.duration_seconds)}s` : '-'}
                  </TableCell>
                  <TableCell>
                    {typeof job.total_input_tokens === 'number' || typeof job.total_output_tokens === 'number' ? (
                      <>
                        {(job.total_input_tokens || 0) + (job.total_output_tokens || 0)}
                        {(() => {
                          const input = job.total_input_tokens || 0;
                          const output = job.total_output_tokens || 0;
                          const inputCost = (input / 1000000) * 3; // $3 per 1M input tokens
                          const outputCost = (output / 1000000) * 15; // $15 per 1M output tokens
                  ...
                  <TableCell>{job.created_at ? formatDate(job.created_at) : '-'}</TableCell>
                  <TableCell>{job.updated_at ? formatDate(job.updated_at) : '-'}</TableCell>
                  <TableCell>
                    <Box
                      sx={{
                        display: 'flex',
